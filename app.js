...
els.btnDeleteFromDetail?.addEventListener("click", deleteCurrentCustomer);
  ...
  function renderBudgetItems(){ const items=state.budgetUi.draftItems||[]; setText(els.budgetTotal, euro(computeBudgetTotal())); setHTML(els.budgetItems, items.map((it)=>`<div class="grid2"><input data-budget-concept="${it.id}" placeholder="Concepto" value="${escapeHtml(it.concept||"")}"/><input data-budget-quantity="${it.id}" type="number" min="0.01" step="0.01" value="${escapeHtml(it.quantity)}"/><input data-budget-unit="${it.id}" type="number" min="0" step="0.01" value="${escapeHtml(it.unit_price)}"/><div>Total línea: <strong>${escapeHtml(euro(budgetLineTotal(it)))}</strong> <button class="btn btn-ghost" data-budget-remove="${it.id}" type="button">Quitar</button></div></div>`).join("")); }
  function onBudgetItemsInput(event){ const t=event.target; const id=t.dataset.budgetConcept||t.dataset.budgetQuantity||t.dataset.budgetUnit; if(!id) return; const it=state.budgetUi.draftItems.find(x=>x.id===id); if(!it) return; if(t.dataset.budgetConcept) it.concept=t.value; if(t.dataset.budgetQuantity) it.quantity=t.value; if(t.dataset.budgetUnit) it.unit_price=t.value; renderBudgetItems(); }
  ...
  async function saveBudget(){ const items=state.budgetUi.draftItems.map(it=>({concept:normalize(it.concept), quantity:clampMoney(it.quantity), unit_price:clampMoney(it.unit_price)})); const err=validateBudgetItems(items); if(err) return showToast(err,'warning'); const total=clampMoney(items.reduce((s,it)=>s+clampMoney(it.quantity*it.unit_price),0)); const payload={customer_id:state.budgetUi.selectedCustomerId,presupuesto_date:normalize(els.budgetDate?.value)||todayISO(),valid_until:normalize(els.budgetValidUntil?.value)||null,status:normalize(els.budgetStatus?.value)||'draft',comments:normalize(els.budgetComments?.value),internal_notes:normalize(els.budgetInternalNotes?.value),total_amount:total}; let saved=null; if(state.budgetUi.editingId){ const {data,error}=await withTimeout(supabase.from('presupuestos').update(payload).eq('id',state.budgetUi.editingId).select('*').single(),12000); if(error) throw error; saved=data; await withTimeout(supabase.from('presupuesto_items').delete().eq('presupuesto_id',saved.id),12000);} else { const {data,error}=await withTimeout(supabase.from('presupuestos').insert(payload).select('*').single(),12000); if(error) throw error; saved=data; } const ins=items.map(it=>({...it,presupuesto_id:saved.id})); const {error:itErr}=await withTimeout(supabase.from('presupuesto_items').insert(ins),12000); if(itErr) throw itErr; const {data:full,error:fullErr}=await withTimeout(supabase.from('presupuestos').select('*').eq('id',saved.id).single(),12000); if(fullErr) throw fullErr; await createAndAttachBudgetPdf(full, items); await fetchBudgets(); renderBudgetList(); resetBudgetForm({keepOpen:false}); }
  async function createAndAttachBudgetPdf(budget, items){ const svc=window.PresupuestoPdfService; const jspdfCtor=window.jspdf?.jsPDF; if(!budget?.id||!budget?.presupuesto_code||!svc||!jspdfCtor) return null; const customer=state.customerMap.get(budget.customer_id)||null; const company=state.companyMapByCustomerId.get(budget.customer_id)||null; const doc=svc.renderPdfToJsPdf({...budget,items,customer_name: customer?customerDisplayName(customer,company):'Cliente',customer_phone:customer?.phone||'',customer_address: customer?.is_company ? [company?.address, company?.postal_code, company?.city, company?.province].filter(Boolean).join(', ') : (customer?.address||'')}, jspdfCtor); const blob=doc.output('blob'); const fileName=svc.buildFileName(budget.presupuesto_code); const filePath=`budgets/${budget.id}/${fileName}`; const {error:up}=await withTimeout(supabase.storage.from(STORAGE_BUCKET).upload(filePath,blob,{upsert:true,contentType:'application/pdf'}),25000); if(up) throw up; let attachmentId=budget.main_attachment_id||null; const payload={customer_id:budget.customer_id||null,file_name:fileName,file_path:filePath,mime_type:'application/pdf',attachment_kind:'presupuesto_pdf',generated_by_system:true,presupuesto_id:budget.id}; if(attachmentId){ const {error}=await withTimeout(supabase.from('attachments').update(payload).eq('id',attachmentId),12000); if(error) throw error; } else { const {data,error}=await withTimeout(supabase.from('attachments').insert(payload).select('id').single(),12000); if(error) throw error; attachmentId=data?.id||null; } const {error:updateError}=await withTimeout(supabase.from('presupuestos').update({pdf_file_path:filePath,main_attachment_id:attachmentId,pdf_generated_at:new Date().toISOString()}).eq('id',budget.id),12000); if(updateError) throw updateError; }
  ...
  function bindPart2Events() {
    ...
    els.btnBudgetAddItem?.addEventListener("click", () => { state.budgetUi.draftItems.push({ id: uid(), concept: "", quantity: 1, unit_price: 0 }); renderBudgetItems(); });
